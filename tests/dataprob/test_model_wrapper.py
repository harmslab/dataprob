...
def test_init(binding_curve_test_data):
    ...
    assert isinstance(mw.K1,dataprob.FitParameter)
    assert isinstance(mw.K2,dataprob.FitParameter)
    ...
    mw = dataprob.ModelWrapper(model_to_test_wrap,fittable_params=["K1"])
    assert isinstance(mw.K1,dataprob.FitParameter)
    assert not isinstance(mw.K2,dataprob.FitParameter)
    assert not isinstance(mw.extra_stuff,dataprob.FitParameter)

    # Make sure we can pass more than one fittable parameters
    mw = dataprob.ModelWrapper(model_to_test_wrap,fittable_params=["K1","K2"])
    assert isinstance(mw.K1,dataprob.FitParameter)
    assert isinstance(mw.K2,dataprob.FitParameter)
    assert not isinstance(mw.extra_stuff,dataprob.FitParameter)
    assert not isinstance(mw.K3,dataprob.FitParameter)
    ...
    # be used.
    mw = dataprob.ModelWrapper(model_to_test_wrap,fittable_params=["K3","K2"])
    assert not isinstance(mw.K1,dataprob.FitParameter)
    assert isinstance(mw.K2,dataprob.FitParameter)
    assert not isinstance(mw.extra_stuff,dataprob.FitParameter)
    assert isinstance(mw.K3,dataprob.FitParameter)
    assert mw.K3.guess == 42

    # Recognizes bad manually passed parameter
    with pytest.raises(ValueError):
        mw = dataprob.ModelWrapper(model_to_test_wrap,fittable_params=["not_real"])

    # Recognizes another type of bad manually passed parameter
    with pytest.raises(ValueError):
        mw = dataprob.ModelWrapper(model_to_test_wrap,fittable_params=["extra_stuff"])

    # pass model that uses a reserved name as an argument
    with pytest.raises(ValueError):
        mw = dataprob.ModelWrapper(bad_model_with_reserved_name)
